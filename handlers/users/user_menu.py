...
wrapper = Sms('Ad860d16567A94Aedbd8872d7eA6774c')
...
async def send_danet(call: CallbackQuery):
    ...
    def telega():
        available_phones = GetFreeSlots(country=SmsTypes.Country.US).request(wrapper)
...
async def genene(call: CallbackQuery, state: FSMContext):
    chat_id = call.message.chat.id
    await call.answer("❗В ближайшее время допишу")
    await call.answer("❗В ближайшее время допишу")
    await call.answer("❗В ближайшее время допишу")
    await call.answer("❗В ближайшее время допишу")
    await call.answer("❗В ближайшее время допишу")
